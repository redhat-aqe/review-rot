...
class PhabricatorTest(TestCase):
    ...
    def test_differential_query_successful(self):
        ...
        fake_status = "Open"
        fake_responsible_users = ["PHID-1", "PHID-2"]

        # Call function
        response = PhabricatorService().differential_query(
            status=fake_status,
            responsible_users=fake_responsible_users,
            phab=self.fake_phab,
        )

        # Validate function calls and response
        self.fake_phab.differential.query.assert_called_with(
            status=fake_status, responsibleUsers=fake_responsible_users
        )
    ...
    def test_user_query_ids_successful(self):
        ...
        response = PhabricatorService().user_query_ids(
            phids=fake_phids, phab=self.fake_phab
        )

        # Validate function calls and response
        self.fake_phab.user.query.assert_called_with(phids=fake_phids)
        self.assertEqual(expected_response, response)
    ...
    def test_user_query_usernames_successful(self):
        ...
        response = PhabricatorService().user_query_usernames(
            usernames=fake_usernames, phab=self.fake_phab
        )

        # Validate function calls and response
        self.fake_phab.user.query.assert_called_with(usernames=fake_usernames)
        self.assertEqual(expected_response, response)

    @patch(PATH + "PhabricatorService.user_query_ids")
    def test_author_data_in_raw_reponse(self, mock_user_query_ids):
        """Tests author_data function where author_phid IS IN the raw response."""
        # Set up mock return values and side effects
        fake_user, fake_raw_response = self.mock.author_data_raw_response()

        # Call function
        user, raw_response = PhabricatorService().author_data(
            author_phid=fake_user["phid"], raw_response=fake_raw_response, phab=None
        )
    ...
    @patch(PATH + "PhabricatorService.user_query_ids", return_value=["New User!"])
    def test_author_data_not_in_raw_response(self, mock_user_query_ids):
        ...
        fake_user, _ = self.mock.author_data_raw_response()

        # Call function
        user, raw_response = PhabricatorService().author_data(
            author_phid=fake_user["phid"], raw_response=[], phab=None
        )

        # Validate function calls and response
        mock_user_query_ids.assert_called_with([fake_user["phid"]], None)
        self.assertEqual(expected_user, user)
        self.assertEqual(expected_raw_response, raw_response)

    @patch(PATH + "PhabricatorService.user_query_ids", side_effect=InternetError)
    def test_author_data_failure(self, mock_user_query_ids):
        """
        Tests author_data function.

        Where user_query_ids throws an error (phab APIError)
        """
        # Set up mock return values and side effects
        fake_user, _ = self.mock.author_data_raw_response()
    ...
    @patch(
        PATH + "PhabricatorService.author_data",
        return_value=({"userName": "dummy-user"}, []),
    )
    @patch(PATH + "PhabricatorService.time_from_epoch", return_value="dummy-time")
    @patch(PATH + "LastComment")
    def test_get_last_comment_successful_single(
        self, mock_last_comment, mock_time_from_epoch, mock_author_data
    ):
        """
        Tests get_last_comment function.

        (successful, fake_comments have one comment)
        """
        # Set up mock return values and side effects
        ...
        response = PhabricatorService().get_last_comment(
            comments=fake_comments, phab=None, raw_response=[]
        )

        # Validate function calls and response
        mock_author_data.assert_called_with(
            author_phid="PHID-USER-xxxxxxxxxxxxxxxxxxxx", phab=None, raw_response=[]
        )
        mock_time_from_epoch.assert_called_with("1551763640")
        mock_last_comment.assert_called_with(
            author="dummy-user", body="This is some content", created_at="dummy-time"
        )
    ...
    @patch(PATH + "PhabricatorService.time_from_epoch", return_value="dummy-time")
    @patch(PATH + "LastComment")
    def test_get_last_comment_successful_multiple(
        self, mock_last_comment, mock_time_from_epoch, mock_author_data
    ):
        """
        Tests get_last_comment function.

        (successful, fake_comments have 3 comments)
        """
        # Set up mock return values and side effects
        mock_last_comment.return_value = MagicMock()
        ...
        # Validate function calls and response
        mock_author_data.assert_called_with(
            author_phid="PHID-USER-xxxxxxxxxxxxxxxxxxx3", phab=None, raw_response=[]
        )
        mock_time_from_epoch.assert_called_with("1551763640")
        self.assertEqual(response, mock_last_comment.return_value)

    @patch(PATH + "PhabricatorService.author_data", side_effect=InternetError)
    @patch(PATH + "PhabricatorService.time_from_epoch", return_value="dummy-time")
    def test_get_last_comment_failure(self, mock_time_from_epoch, mock_author_data):
        """
        Tests get_last_comment function.

        (failure, author_data throws exception)
        """
        # Set up mock return values and side effects
        fake_comments = self.mock.get_comments_many()
    ...
    def test_get_comments_successful(self):
        ...
        # Call function
        response = PhabricatorService().get_comments(id=0, phab=self.fake_phab)

        # Validate function calls and response
        self.fake_phab.differential.getrevisioncomments.assert_called_with(ids=[0])
        self.assertEqual(expected_response, response)

    def test_get_comments_failure(self):
        """Tests get_comments function (failure, phab fails)."""
        # Set up mock return values and side effects
        self.fake_phab.differential.getrevisioncomments.side_effect = InternetError

        # Call function
        with self.assertRaises(InternetError):
            PhabricatorService().get_comments(id=0, phab=self.fake_phab)
    ...
    def test_generate_phids_successful(self, mock_user_query_usernames):
        """
        Tests generate_phids function.

        (successful, user_query_usernames does not fail)
        """
        # Set up mock return values and side effects
        (
            expected_list_of_phids,
        ...
        self.assertEqual(expected_raw_response, raw_response)
        mock_user_query_usernames.assert_called_with("user_names", None)

    @patch(PATH + "PhabricatorService.user_query_usernames", side_effect=InternetError)
    def test_generate_phids_failure(self, mock_user_query_usernames):
        """
        Tests generate_phids function.

        (successful, user_query_usernames does fail)
        """
        # Call function
        with self.assertRaises(InternetError):
            list_of_phids, raw_response = PhabricatorService().generate_phids(
    ...
    def test_get_reviews_successful_no_last_comment(
        self,
        mock_phabricator_review,
        mock_author_data,
        ...
        """
        # Set up mock return values and side effects
        mock_phabricator_review.return_value = MagicMock()
        ...
        mock_author_data.assert_called_with(
            "PHID-USER-xxxxxxxxxxxxxxxxxxxx", [], self.fake_phab
        )
        self.assertEqual(response, [mock_phabricator_review.return_value])
    ...
    @patch(PATH + "PhabricatorService.author_data")
    @patch(PATH + "PhabricatorReview")
    def test_get_reviews_successful_with_last_comment(
        self,
        mock_phabricator_review,
        mock_author_data,
        mock_has_new_comments,
        ...
        mock_time_from_epoch.assert_called_with("mock_date")
        mock_check_request_state.assert_called_with("mock_date", self.mock_age)
        mock_has_new_comments.assert_called_with("mock_created_at", True)
    ...
    @patch(PATH + "PhabricatorService.author_data")
    @patch(PATH + "PhabricatorReview")
    def test_get_reviews_successful_request_state(
        self,
        mock_phabricator_review,
        mock_author_data,
        mock_has_new_comments,
    ...
    def test_request_reviews_user_names(
        self,
        mock_get_reviews,
        ...
        response = PhabricatorService().request_reviews(
            host="https://www.dummy.com", token="dummy_token", user_names="test_user"
        )

        # Validate function calls and response
        mock_generate_phids.assert_called_with("test_user", self.fake_phab)
        mock_differential_query.assert_called_with(
            status="status-open", responsibleUsers=[], phab=self.fake_phab
        )
        mock_phabricator.assert_called_with(
            host="https://www.dummy.com/api/", token="dummy_token"
        )
    ...
    @patch(PATH + "PhabricatorService.differential_query")
    @patch(PATH + "PhabricatorService.get_reviews")
    def test_request_reviews_no_user_names(
        self,
        mock_get_reviews,
        mock_differential_query,
        mock_generate_phids,
        ...
        # Call function
        response = PhabricatorService().request_reviews(
            host="https://www.dummy.com", token="dummy_token", user_names=None
        )
    ...
    @patch(PATH + "PhabricatorService.differential_query")
    @patch(PATH + "PhabricatorService.get_reviews")
    def test_request_reviews_no_response(
        self,
        mock_get_reviews,
        mock_differential_query,
        mock_generate_phids,
