...
class BaseServiceTest(TestCase):
    ...
    def test_has_new_comments_none(self):
        """Tests 'has_new_comments' function where last_activity and days are none."""
        # Call the function
        response = BaseService().has_new_comments(last_activity=None, days=None)

        # Assert values were called correctly
        self.assertFalse(response)

    def test_has_new_comments(self):
        """Tests 'has_new_comments' function where last_activity & days is NOT none."""
    ...
    def test_get_response(self):
        ...
        response = service.get_response(
            method="mock_method", url="mock_url", ssl_verify=True
        )
        ...
        mock_session.request.assert_called_with(
            method="mock_method", url="mock_url", headers="mock_header", verify=True
        )
...
class BaseReviewTest(TestCase):
    ...
    def test_format_oneline_one_comment(self, mock_format_duration):
        ...
        # Call the function
        response = service._format_oneline(1, 1)

        # Validate function calls and response
        self.assertEqual(
            "mock_user filed 'mock_title' " "mock_url mock_duration ago, 1 comment",
            response,
        )

    @patch(PATH + "BaseReview.format_duration")
    def test_format_oneline_last_comment(self, mock_format_duration):
        """
        Tests '_format_oneline' function.

        With more than one comment and last comments.
        """
        # Set up mock return values and side effects
        service = self.mock_base_review
        ...
        # Validate function calls and response
        self.assertEqual(
            "mock_user filed 'mock_title' mock_url"
    ...
    @patch(PATH + "BaseReview.format_duration")
    def test_format_indented_one_comment(self, mock_format_duration):
        ...
        # Call the function
        response = service._format_indented(1, 1, False)

        # Validate function calls and response
        self.assertEqual(
            "mock_user filed 'mock_title'\n"
            "\tmock_url\n\tmock_duration ago\n"
            "\t1 comment",
            response,
        )
    ...
    @patch(PATH + "BaseReview.format_duration")
    def test_format_indented_last_comment(self, mock_format_duration):
        """Tests '_format_indented' function with multiple comment and last comments."""
        # Set up mock return values and side effects
        service = self.mock_base_review
        self.mock_base_review.comments = 10
        mock_last_comment = MagicMock()
        ...
        # Call the function
        response = service._format_indented(1, 1, True)
    ...
    @patch(PATH + "BaseReview.format_duration")
    def test_format_irc_one_comment(self, mock_format_duration):
    ...
    @patch(PATH + "BaseReview.format_duration")
    def test_format_irc_last_comment(self, mock_format_duration):
        """Tests '_format_irc' function with more than one comment and last comments."""
        # Set up mock return values and side effects
        service = self.mock_base_review
        self.mock_base_review.comments = 10
        mock_last_comment = MagicMock()
        ...
        # Validate function calls and response
        self.assertEqual(
            "\x02mock_user\x02 filed \x02'mock_title'\x02"
